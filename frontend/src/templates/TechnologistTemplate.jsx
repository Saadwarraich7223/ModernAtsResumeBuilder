  ...
  return (
    ...
    >
      <div className="grid grid-cols-12 min-h-[297mm] text-left">
        ...
        <div
          className="col-span-4 bg-[#1e293b] border-r border-slate-700 text-left"
          style={{ padding: settings.pageMargin }}
        >
          <div className="mb-10 text-left">
            <h1 className="text-3xl font-black text-white tracking-tighter mb-2 uppercase text-left">
              {personalInfo.fullName || "Name"}
            </h1>
            <p
              className="text-xs font-bold uppercase tracking-[0.2em] text-left"
              style={{
                color: theme.primary.includes("gradient")
                  ? theme.accent
                  : theme.primary,
              }}
            >
          ...
          <div className="space-y-10 text-left">
            <section className="text-left">
              ...
              <div className="flex flex-wrap gap-2 text-left">
                {skills.map((skill, i) => (
                  <span
                    key={i}
                    className="px-2 py-1 bg-slate-800 text-slate-200 border border-slate-700 rounded font-mono text-[9px] font-bold uppercase text-left"
                  >
                    {skill.name}
                  </span>
                ))}
            ...
            {isVisible("languages") && languages?.length > 0 && (
              <section className="text-left">
                <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 mb-4 text-left">
                  Linguistics.log
                </h2>
                ...
                <div className="space-y-4 text-left">
                  {certifications.map((cert, i) => (
                    <div key={i} className="text-left">
                      <p className="text-[10px] font-black text-white leading-tight uppercase text-left">
                        {cert.name}
                      </p>
                      <p className="text-[9px] font-bold opacity-50 uppercase text-left mt-1">
                        {cert.issuer}
                      </p>
                    </div>
                  ))}
            ...
            <section className="text-left">
              ...
              <div className="space-y-2 text-[10px] font-mono opacity-80 uppercase tracking-tight text-left">
                {personalInfo.phone && (
                  <p className="break-all">PHONE: {personalInfo.phone}</p>
                )}
                {personalInfo.email && (
                ...
                {personalInfo.linkedin && (
                  <p className="break-all">LINKEDIN: {personalInfo.linkedin}</p>
                )}
                {personalInfo.website && (
                  <p className="break-all">PORTFOLIO: {personalInfo.website}</p>
                )}
              </div>
            </section>
          </div>
        ...
        <div
          className="col-span-8 text-left"
          style={{ padding: settings.pageMargin }}
        >
          {summary && (
            <section className="mb-12 text-left">
              ...
              <p className="text-slate-300 font-medium leading-relaxed font-mono text-sm opacity-90 text-left">
                {summary}
              </p>
            </section>
          )}
            ...
            <section className="mb-12 text-left">
              ...
              <div className="space-y-10 text-left">
                {workExperience.map((exp, i) => (
                  <div
                    key={i}
                    className="relative pl-6 border-l border-slate-800 text-left"
                  >
                    <div
                      className="absolute w-2 h-2 bg-slate-700 rounded-full -left-[4.5px] top-1.5"
                      style={{ background: theme.accent }}
                    ></div>
                    <div className="flex justify-between items-baseline mb-1 text-left">
                      <h3 className="text-white font-black font-mono tracking-tight uppercase text-left">
                        {exp.position}
                      </h3>
                      <span className="text-[9px] font-mono text-slate-500 font-bold uppercase text-left">
                        {exp.startDate} - {exp.endDate}
                      </span>
                    </div>
                    <p
                      className="text-[10px] font-bold mb-4 uppercase tracking-widest text-left"
                      style={{
                        color: theme.primary.includes("gradient")
                          ? theme.accent
                          : theme.primary,
                    ...
                    <p className="text-slate-400 text-xs font-mono leading-relaxed whitespace-pre-wrap opacity-80 text-left">
                      {exp.description}
                    </p>
                  </div>
                ))}
                ...
                {projects.map((project, i) => (
                  ...
                  >
                    ...
                    {project.technologies && (
                      <p className="text-[10px] font-bold mb-2 uppercase tracking-widest opacity-50 text-left">
          ...
          {education?.length > 0 && (
            <section className="mb-12 text-left">
              ...
              <div className="space-y-4 font-mono text-left">
                {education.map((edu, i) => (
                  <div key={i} className="text-left">
                    <p className="text-white font-black text-xs uppercase tracking-tight text-left">
                      {edu.degree}
                    </p>
                    <p className="text-slate-500 text-[10px] font-bold uppercase mt-1 text-left">
          ...
          {isVisible("custom") &&
            customSections?.map((section, i) => (
              <section key={i} className="mb-12 text-left">
                ...
                <p className="text-slate-400 text-xs font-mono leading-relaxed whitespace-pre-wrap text-left">
                  {section.content}
                </p>
              </section>
            ))}
